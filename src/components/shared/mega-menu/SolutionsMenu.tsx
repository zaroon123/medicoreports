// components/shared/mega-menu/SolutionsMenu.tsx
...
const SolutionsMenu = ({ 
  ...
  // State: Which submenu is active? Default to null (no active submenu until hover)
  const [activeSubmenu, setActiveSubmenu] = useState<string | null>(null);
  const [isHoveringDropdown, setIsHoveringDropdown] = useState(false);
  const timeoutRef = useRef<NodeJS.Timeout | null>(null);

  // Effect to set default submenu when Solutions is hovered
  useEffect(() => {
    if (isParentHovered) {
      // Only set default if no active submenu is set
      if (!activeSubmenu) {
        setActiveSubmenu('medico-legal');
      }
    } else {
  ...
  const activeItem = solutionsMenuItems.find(item => item.id === activeSubmenu);
  ...
  // Split submenu items into two columns for Other Sectors
  ...
  const getSubmenuHeader = () => {
    if (activeItem?.id === 'medico-legal') {
      return {
        title: activeItem.label,
        subtitle: 'Medico Legal Solutions'
      };
